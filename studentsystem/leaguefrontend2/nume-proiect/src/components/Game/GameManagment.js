...
import './GameManagement.css'; // Import the CSS file
...
function GameManagement() {
  ...
  const createGame = () => {
    if (!isAdmin) {
      alert('Only admins can create games.');
      return;
    }
  ...
  const simulateGame = (gameId) => {
    if (!isAdmin) {
      alert('Only admins can simulate games.');
      return;
    }

    if (selectedGameId !== gameId) { // Check to prevent duplicate simulations
      setSelectedGameId(gameId);
      setSimulationResults([]); // Reset simulation results
        ...
        <div className="container mx-auto p-4">
          <div className="flex flex-col md:flex-row md:justify-between mb-8">
            <div className="w-full md:w-1/2 p-4 bg-white rounded-lg shadow-md">
              ...
              <input
                ...
                style={{ color: '#333' }}
              />
              <button onClick={createGame} className="w-full p-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition duration-300" disabled={!isAdmin}>
                Create Game
              </button>
              {!isAdmin && <p className="text-red-500">Only admins can create games.</p>}
            </div>
            {selectedGameId && (
          ...
          <div className="flex flex-col md:flex-row md:justify-between">
            <div className="w-full md:w-1/2 p-4 bg-white rounded-lg shadow-md">
                  ...
                  {scheduledGames.map((game, index) => (
                    <tr key={index} className="bg-gray-100 border-b border-gray-200 hover:bg-gray-200">
                      ...
                      <td className="px-4 py-2">
                        <button onClick={() => simulateGame(game.id)} className="px-2 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 transition duration-300" disabled={!isAdmin}>
                          Simulate
                        </button>
                      </td>
                    </tr>
              ...
              {!isAdmin && <p className="text-red-500">Only admins can simulate games.</p>}
            </div>
            <div className="w-full md:w-1/2 p-4 bg-white rounded-lg shadow-md mt-8 md:mt-0">
